<?php
	...
	if(isset($_POST["photo_submit"])){
		...
		if($check !== false){
			//kontrollime, kas aktepteeritud failivorming ja fikseerime laiendi NB! JPGE
			if($check["mime"] == "image/jpeg"){
				$image_file_type = "jpg";
			} elseif ($check["mime"] == "image/png"){
				$image_file_type = "png";
			} else {
				$photo_upload_error = "Formaat ei sobi! Ainult jpg ja png on lubatud!";
			}
		} else {
		...
		if(empty($photo_upload_error)){
			...
			//suuruse muutmine
				...
				$new_temp_image = resize_image($temp_image, 600, 400, true); //propotsiooni säilitamiseks
				
				$target_file = "../upload_photos_normal/" .$image_file_name; //formaadi kontroll
				if($image_file_type == "jpg"){
					if(imagejpeg($new_temp_image, $target_file, 90)){
						$photo_upload_error = "Väiksemkas tehtud pilt on salvestatud!"; 
					} else {
						$photo_upload_error = "Väiksemaks tehtud pilti ei salvestatud!";
					}
				}
				if($image_file_type == "png"){
					if(imagepng($new_temp_image, $target_file, 6)){
						$photo_upload_error = "Väiksemaks tehtud pilt on salvestatud!";
					} else {
						$photo_upload_error = "Väiksemaks tehtud pilti ei salvestatud!";
					}
				}
				...
				//algpildi salvestamine enne neljandat kodutööd
