<?php
  ...
  //value="<?php echo isset($_POST["news_title_input"]) ? $_POST["news_title_input"] : "" //lõpetad ka php siin ära
  ...
  if(isset($_POST["user_data_submit"])){
	...
	//kuupäeva reaalsus, valiidsus
	if(empty($birth_day_error)and empty($birth_month)and empty($birth_year_error)){
	  ...
	  if(empty($_POST["confirmpassword_input"])){
		...
		if ($_POST["confirmpassword_input"] !=$_POST["password_input"]){
			$confirm_password_error = "Salasõnad ei ühti!";
		}
		
	}

	//kui vigu pole, siis salvesta
